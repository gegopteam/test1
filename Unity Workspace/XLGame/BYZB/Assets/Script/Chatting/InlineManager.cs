...
public class InlineManager : MonoBehaviour {
    ...
    #endregion
	//清除已绘制的图片，from芦
	public void ClearSprite(int _id){
		SpriteGraphic _myspriteGraphic = _IndexSpriteGraphic[_id]._SpriteGraphic;
		Mesh _mymesh = _IndexSpriteGraphic[_id]._Mesh;;
		_mymesh.Clear();
		_myspriteGraphic.canvasRenderer.SetMesh(_mymesh);
		_myspriteGraphic.UpdateMaterial();
		Debug.LogError ("clear2");
