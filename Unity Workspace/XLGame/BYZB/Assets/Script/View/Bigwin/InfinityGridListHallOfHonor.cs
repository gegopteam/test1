...
public class InfinityGridListHallOfHonor : MonoBehaviour 
{
    ...
    public delegate void UpdateChildrenCallbackDelegateNew(int index, Transform trans);
    public UpdateChildrenCallbackDelegateNew updateChildrenCallbackNew = null;
    ...
    IEnumerator InitChildren()
    {
        ...
        for (int index = 0; index < transform.childCount; index++)
        {
            ...
            //初始化前面几个;
            UpdateChildrenCallbackNew(children.Count - 1, transform.GetChild(index));
        }
                ...
                //向上拉，向下扩展;
                {
                    ...
                    if (childBottom >= scrollRectUp)
                    {
                        ...
                        for (int index = 0; index < gridLayoutGroup.constraintCount; index++)
                        {
                            ...
                            {
                                UpdateChildrenCallbackNew(realIndex, children[index]);
                            }
                        }
            ...
            else
            {
                ...
                if (childUp < scrollRectBottom)
                {
                    ...
                    for (int index = 0; index < gridLayoutGroup.constraintCount; index++)
                    {
                        ...
                        UpdateChildrenCallbackNew(realIndex - children.Count - index, children[children.Count - 1 - index]);
                    }
    ...
    void UpdateChildrenCallbackNew(int index,Transform trans)
    {
        if (updateChildrenCallbackNew != null)
        {
            updateChildrenCallbackNew(index, trans);
        }
    }
