...
public class InfinityGridListMyDuan : MonoBehaviour
{
    ...
    public delegate void UpdateChildrenDelegateMyDuan(int index, Transform trans);
    public UpdateChildrenDelegateMyDuan updateChildrenCallMyDuan = null;
    ...
    IEnumerator InitChildren()
    {
        ...
        for (int index = 0; index < transform.childCount; index++)
        {
            ...
            //初始化前面几个;
            UpdateChildrenCallMyDuan(children.Count - 1, transform.GetChild(index));
        }
                ...
                //向上拉，向下扩展;
                {
                    ...
                    if (childBottom >= scrollRectUp)
                    {
                        ...
                        for (int index = 0; index < gridLayoutGroup.constraintCount; index++)
                        {
                            ...
                            {
                                UpdateChildrenCallMyDuan(realIndex, children[index]);
                            }
                        }
            ...
            else
            {
                ...
                if (childUp < scrollRectBottom)
                {
                    ...
                    for (int index = 0; index < gridLayoutGroup.constraintCount; index++)
                    {
                        ...
                        UpdateChildrenCallMyDuan(realIndex - children.Count - index, children[children.Count - 1 - index]);
                    }
    ...
    void UpdateChildrenCallMyDuan(int index, Transform trans)
    {
        if (updateChildrenCallMyDuan != null)
        {
            updateChildrenCallMyDuan(index, trans);
        }
    }
