...
public class ActivityTow : MonoBehaviour
{
	[SerializeField]
	...
	IEnumerator AnalysisJson (string url, Dictionary<string, string> formDic)
	{
		...
		if (_www.isDone && _www.error == null) {
			...
			btnText.text = link_btn_text [1];
			StartCoroutine (GetImageOne (datalength [1], image.sprite));
	...
	public void OnActivityWindowClick ()
	{
		...
		GameObject activity = Resources.Load<GameObject> ("Window/Activity_2");
		...
		if (userInfo.platformType == 22 || userInfo.platformType == 24) {
			tempStr = jump_url [1] + "?UserID=" + userInfo.userID + "&pwd=" + userInfo.mLoginData.token + "&type=" + userInfo.GetTokenType () + "&uname=" + WWW.EscapeURL (userInfo.nickname);
		} else {
			tempStr = jump_url [1] + "?UserID=" + userInfo.userID + "&pwd=" + userInfo.password + "&type=" + 0 + "&uname=" + WWW.EscapeURL (userInfo.nickname);
		}
	...
	{
		GameObject activity = Resources.Load<GameObject> ("Window/Canvas_3");
		GameObject go_1 = Instantiate (activity);
